...
prediction2_train = bc.predict(X_train)
prediction2 = bc.predict(X_test)
...
combined_predictions_train = \
    np.array([
        ...
        [prediction2_train[a], prediction3_train[a]] \
...
vote_predictions = \
    np.array([
        ...
        [prediction2[a], prediction3[a], prediction1[a]].count(1) >= 2
        for a in range(len(prediction1))
    ])
...
p2_train = BC().fit(X,Y).predict(X)
p3_train = DTC().fit(X,Y).predict(X)

p1 = GBC().fit(X,Y).predict(test_data)
p2 = BC().fit(X,Y).predict(test_data)
p3 = DTC().fit(X,Y).predict(test_data)
...
create_submission('results_GBC_BC.txt', output)
